...
"""
Current Objectives:
- Error checking to prevent a person from being scheduled more than once on the same day
- Correctly report jobs that people are being assigned to (Sometims a PIC2 person is being reported as assigned to
a PIC2 job even though they've been assigned to a PIC1 job)
- Fix randomizing algorithm: error issues and allocating is still wonky
- Add Holiday checking
- Export results to Excel
- Work on developing an equalization function for the scoring function
"""
...
def create_solution(employee_bank, month_calendar):
    ...
    num_assigned_pic1jobs = 0
    for employee in assigned_jobs_PIC1:
        for jobs in employee[1]:
            num_assigned_pic1jobs += 1
    ...
    num_assigned_pic1pjobs = 0
    for employee in assigned_jobs_PIC1P:
        for jobs in employee[1]:
            num_assigned_pic1pjobs += 1
    ...
    num_assigned_pic2jobs = 0
    for employee in assigned_jobs_PIC2:
        for jobs in employee[1]:
            num_assigned_pic2jobs += 1

    print "Assigned PIC1 jobs: ", assigned_jobs_PIC1
    print "Number of PIC1 jobs: ", num_assigned_pic1jobs
    print "Assigned PIC1+ jobs: ", assigned_jobs_PIC1P
    print "Number of PIC1+ jobs: ", num_assigned_pic1pjobs
    print "Assigned PIC2 jobs: ", assigned_jobs_PIC2
    print "Number of assigned PIC2 jobs: ", num_assigned_pic2jobs
    print "Leftover shifts: ", leftover_employees
